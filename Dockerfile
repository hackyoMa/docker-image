...
RUN chmod +x /usr/local/run-java.sh && \
    ...
    mkdir ${JAVA_HOME} && \
    curl -L https://github.com/adoptium/temurin8-binaries/releases/download/jdk${TEMURIN_VERSION}/OpenJDK8U-jre_${DOWNLOAD_ARCH}_linux_hotspot_${JAVA_VERSION}.tar.gz -o ${JAVA_HOME}/jdk.tar.gz && \
    tar -xf ${JAVA_HOME}/jdk.tar.gz -C ${JAVA_HOME} && \
    mv ${JAVA_HOME}/jdk${TEMURIN_VERSION}-jre/* ${JAVA_HOME}/ && \
    rm -r ${JAVA_HOME}/jdk${TEMURIN_VERSION}-jre ${JAVA_HOME}/jdk.tar.gz
CMD ["java", "-version"]
