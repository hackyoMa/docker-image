...
ARG TARGETPLATFORM
ENV ZULU_VERSION_X64=11.66.19 ZULU_VERSION_AARCH64=11.66.19 JAVA_VERSION=11.0.20.1 JAVA_HOME=/usr/java/openjdk-11 JAVA_OPTIONS=-Dfile.encoding=utf-8
ENV CLASSPATH=${JAVA_HOME}/lib PATH=${PATH}:${JAVA_HOME}/bin
RUN if [ "${TARGETPLATFORM}" = "linux/amd64" ]; then BASE_DOMAIN="zulu" && DOWNLOAD_ARCH="x64" && ZULU_VERSION=${ZULU_VERSION_X64}; else BASE_DOMAIN="zulu" && DOWNLOAD_ARCH="aarch64" && ZULU_VERSION=${ZULU_VERSION_AARCH64}; fi && \
    mkdir -p ${JAVA_HOME} && \
